...
class Assistant(object):
    ...
    def get_single_item_stock(self, sku_id, num, area):
        ...
        try:
            ...
            sexprice=resp_json.get('stock').get('jdPrice').get('p')


            print("The price is now " + resp_json.get('stock').get('jdPrice').get('p')) #锁定价格
            with open("tmp.json", "w") as fp:
                fp.write(json.dumps(resp_json, indent=4))
            ...
            return sku_state == 1 and stock_state in (33, 40) and float(sexprice)<20
            #return False
    ...
    def get_multi_item_stock_new(self, sku_ids, area):
        ...
        try:
            resp_text = requests.get(url=url, params=payload, headers=headers, timeout=self.timeout).text
            # print(parse_json(resp_text).items())
            stock = True
    ...
    def if_item_can_be_ordered(self, sku_ids, area):
        ...
        # kio = self.get_item_price(sku_id=sku_id)
        #print(float(kio)>70)
        # 判断商品是否能下单
        if len(items_dict) > 1:
    ...
    @deprecated
    def exec_seckill_by_time(self, sku_ids, buy_time, retry=4, interval=4, num=1, fast_mode=False):
        """定时抢购
    ...
    @check_login
    def exec_reserve_seckill_by_time(self, sku_id, buy_time, retry=500, interval=0.5, num=1):
        """定时抢购`预约抢购商品`
    ...
    @check_login
    def buy_item_in_stock(self, sku_ids, area, wait_all=False, stock_interval=3, submit_retry=10, submit_interval=5):
        """根据库存自动下单商品
